...
from tests import TICKER_RESPONSE, PRICES_RESPONSE, MARKETS_RESPONSE, GET_ACCOUNT_INFO_RESPONSE, CURRENCIES_RESPONSE, \
    ...
    GENERATE_WALLETS_RESPONSE, TICKET_RESPONSE, GET_TICKETS_RESPONSE, REPLY_TICKET_RESPONSE
...
class TestStocksExchangeAPI(TestCase):
    ...
    def test_public_query(self, m):
        ...
        _method_name = 'ticker'
        m.register_uri('GET', STOCK_EXCHANGE_BASE_URL.format(method=_method_name), text=TICKER_RESPONSE)
        data = self.api.query(StockExchangeResponseParser, TickerRequest)
        ...
        with patch('time.time') as time_mock:
            ...
            data = self.api.query(StockExchangeResponseParser, TickerRequest, with_saving=True)
            self.assertEqual(m.call_count, 2)
            self.assertTrue(data)

            time_mock.return_value = 140.0
            data = self.api.query(StockExchangeResponseParser, TickerRequest, saving_time=60.0)
            self.assertEqual(m.call_count, 2)  # no call because time has not passed yet
            self.assertTrue(data)

            # change threshold to 5 seconds
            data = self.api.query(StockExchangeResponseParser, TickerRequest, saving_time=5.0)
    ...
    def test_private_grafic(self, m):
        _method_name = 'private_grafic'
        self.assertPrivateMethod(_method_name, response_data=PRIVATE_GRAFIC_RESPONSE, m=m)
